...
let svgPathAll,
  ...
  realPriceNewNumber,
  realPriceNewNew,
  realPriceNewNewNumber,
  sortedArray,
  priceHightoLow,
  priceHightoLowTwo,
  priceLowToHighSplitOne,
  priceLowToHighSplitTwo,
  sortedPriceHighToLow;
...
sortAnchorTag.addEventListener("click", () => {
  ...
  for (let i = 0; i < 4; i++) {
    const sortByChildThree = document.querySelector(
      ".sort-by-fixed-div-child-three"
    );
  ...
  const sortByFixedChildThree = document.querySelectorAll(
    ".sort-by-fixed-child-three"
  );
  sortByFixedChildThree[1].addEventListener("click", (a) => {
    const sectionMainContainer = document.querySelector(
      ".section-main-container"
    );
    sectionMainContainer.innerHTML = ``;
    let h = 0;
    for (storeIndexLowToHigh[h]; h < 6; storeIndexLowToHigh[h + 1]) {
      let i = parseInt(storeIndexLowToHigh[h]);
      console.log("hiii");
      ...
      if (discount[i] === 0) {
        let requiredIndex = i;
        console.log(i);
        const secondCardThirdChild = document.querySelectorAll(
          ".second-card-third-child-text"
        );
        const marginZero = document.querySelectorAll(
          ".third-child-discount-price"
        );
        const secondCard = document.querySelectorAll(
          ".second-card-third-child-price"
        );
      ...
      if (emiAvailable[i] != "null") {
        const addEmiField = document.querySelectorAll(
          ".card-second-section-container"
        );
        ...
        const secondCardLastChild = document.querySelectorAll(
  ...
  sortByFixedChildThree.forEach((a) => {
...
///////// for price low to high sorting///////////////////////////
priceLowToHighSplitOne = priceHightoLow.map(Number);
console.log(priceLowToHighSplitOne);
priceLowToHighSplitTwo = priceHightoLowTwo.map(Number);
sortedPriceHighToLow = priceLowToHighSplitOne.sort((a, b) => {
  if (a < b) {
    return -1;
  }
  if (a === b) {
...
console.log(priceLowToHighSplitTwo);
console.log(sortedPriceHighToLow);
sortedPriceHighToLow.map((a, i) => {
  for (let g = 0; g < 7; g++) {
    if (a === priceLowToHighSplitTwo[g]) {
      console.log(priceLowToHighSplitOne[g]);
      storeIndexLowToHigh += g;
      console.log(storeIndexLowToHigh);
    }
  }
});
storeIndexLowToHigh = storeIndexLowToHigh.split("");
console.log(storeIndexLowToHigh);
...
for (let i = 0; i < rating.length; i++) {
  const sectionMainContainer = sectionBody.querySelector(
    ".section-main-container"
  );
...
function callStyleFunTwo() {
  for (let i = 0; i < marginsZero.length; i++) {
    let i = parseInt(indexForPriceSorting[d]);
    marginsZero[0].innerHTML = `₹${priceCalc[1]}`;
    marginsZero[1].innerHTML = `₹${priceCalc[0]}`;
    marginsZero[2].innerHTML = `₹${priceCalc[4]}`;
    marginsZero[3].innerHTML = `₹${priceCalc[5]}`;
    marginsZero[4].innerHTML = `${arrayElem[3]}`;
    marginsZero[5].innerHTML = `${arrayElem[2]}`;
  }
}
